...
- `ai_mood` is a presentation-safe diagnostic from the core policy. The fixed
  ...
  movement, player shot, shield integrity, and EMP state. `game_move` records
...
- `bonus.c` derives each room's saucer interval from `game_seed_hash`; Cache
  ...
  interval and direction in core and do not hard-code them in the UI.
...
- Shop rendering should preview the core interest rule (one credit per five
  saved, capped at three) without duplicating or mutating economy logic.
...
- Leaving an untouched shop grants one `skip_rerolls` tag; any purchase, reroll,
  hold, or route choice forfeits the tag, and the next shop consumes it before
  credits.
...
- `module_sell.c` permits one deterministic two-credit sale per shop through
  Shift+1..9, clearing the selected owned module without consuming RNG.
...
- Overclock is a scoring module in `modules.c`: it adds three points before combo
...
- Expose the authoritative cap through `game_interest_cap`; the SDL shop may
  display it but must not duplicate the Banker/Bounty/Static rules.
