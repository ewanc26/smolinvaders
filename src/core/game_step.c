...
static void room_progress(Game *g) { int next=1+g->score/5; if(next>g->room){g->rng=g->rng*1103515245+12345;g->room=next;g->wave=next;g->room_type=(g->rng&0x7fffffff)%3;g->alien_hp=g->room_type==ROOM_ELITE?3:1;g->upgrade_offer=true;g->paused=true;} }
