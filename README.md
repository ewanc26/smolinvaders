...
readily. Abilities can stack into named threats such as Assault, Breacher, Siege,
and Dreadnought; the profile is shown in the HUD and as colored markers on the
invader, and never consumes combat RNG.
...
Sell an owned module with `Shift+1` through `Shift+9` for two credits; selling is
...
The shop displays the active interest cap before you choose whether to spend.
...
Consecutive kills inside a 24-tick window build a combo up to eight; each relic
tier widens that window by two ticks, up to 30. At four and eight kills, the
next scoring event earns a 2x or 3x multiplier. Losing a life breaks the combo,
so chasing saucers can be worth the danger.
